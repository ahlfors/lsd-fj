<template>
  <viewTemplate class="a1-mobile-internetPage" :interval="30" @interval="loadData">
    <div>
    ...
    methods: {
      loadData() {
        ...
        this.$http.indi.get('f0059', {}, (res59) => {
          if (res59.data.data.ACTIVE_USER_PEAK_TIME) {
            this.active_peek = (new Date(Date.parse(res59.data.data.ACTIVE_USER_PEAK_TIME))).pattern('HH时mm分');
          }
          if (res59.data.data.HTTP_AVERAGE_DOWNLOAD_RATE_PEAK_TIME) {
            this.http_avg_peek = (new Date(Date.parse(res59.data.data.HTTP_AVERAGE_DOWNLOAD_RATE_PEAK_TIME))).pattern('HH时mm分');
          }
          if (res59.data.data.HTTP_SUCCESS_RATE_PEAK_TIME) {
            this.http_succ_peek = (new Date(Date.parse(res59.data.data.HTTP_SUCCESS_RATE_PEAK_TIME))).pattern('HH时mm分');
          }
          if (res59.data.data.HTTP_FIRST_PACKET_RESPONSE_DELAY_PEAK_TIME) {
            this.http_delay_peek = (new Date(Date.parse(res59.data.data.HTTP_FIRST_PACKET_RESPONSE_DELAY_PEAK_TIME))).pattern('HH时mm分');
          }
          this.activeUserNum = res59.data.data.NUMBER_OF_ACTIVE_USER;
          this.activeUserPeek = res59.data.data.ACTIVE_USER_PEAK;
          this.downFlow = res59.data.data.DOWNSTREAM_TRAFFIC;
          this.downFlowPeek = res59.data.data.CUMULATIVE_DOWNSTREAM_TRAFFIC;
          this.upFlow = res59.data.data.UPSTREAM_TRAFFIC;
          this.upFlowPeek = res59.data.data.CUMULATIVE_UPSTREAM_TRAFFIC;
          this.httpAvgDown = res59.data.data.HTTP_AVERAGE_DOWNLOAD_RATE;
          this.httpDownPeek = res59.data.data.HTTP_AVERAGE_DOWNLOAD_RATE_PEAK;
          this.httpSuc = res59.data.data.HTTP_SUCCESS_RATE;
          this.httpSucPeek = res59.data.data.HTTP_SUCCESS_RATE_PEAK;
          this.headDelay = res59.data.data.HTTP_FIRST_PACKET_RESPONSE_DELAY;
          this.headDelayMin = res59.data.data.HTTP_FIRST_PACKET_RESPONSE_DELAY_MINIMUM;
        })
        // window.setTimeout(() => {
