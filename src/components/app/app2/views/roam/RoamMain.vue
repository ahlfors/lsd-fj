<!--实时性能-->
<template>
  <div viewId="02" class="a2-roam" @ok = "toTools()">
    ...
    <viewTemplate viewId="02"  @ok = "toTools()" @click="toTools" class="a1-roam-tools"></viewTemplate>
  </div>
</template>
<script>
  ...
  export default {
    ...
    methods: {
      toTools(viewId) {
        if (viewId) this.$store.commit(MutationsBridge.CLICK_VIEW_ID_SSA, viewId);
        this.$router.push({
          name: 'roam/roamIn/xm'
        });
      }
