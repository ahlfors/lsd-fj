<!--传统业务 用户数 网络规模-->
<template>
  <viewTemplate class="a2-traditional-traditionalTop" :interval="30" @interval="loadData">
    ...
    <div class="traditionalTop-userNum">
      ...
      <div>
        <div>FDD用户数</div>
        <div><counter :data="pddRate" :config="indexNumConfig1"></counter></div>
      </div>
    </div>
  </viewTemplate>
</template>
  ...
  export default {
    ...
    methods: {
      loadData() {
        ...
        this.volteRate = 0;
        this.pddRate = 0;
        this.$http.indi.get(['f0100', 'f0101', 'f0102', 'f0288'], {}, (res1, res2, res3, res288) => {
          try {
            ...
            this.pddRate = res288.data.data[0].KPI_VALUE;
          } catch (err) {
            console.log('f0100 - f0102,f0288 indicator has error')
          }
        });
      }
    }
