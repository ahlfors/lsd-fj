...
var GPSTool = {
  ...
  /** GPS坐标转google坐标 */
  gpsToGoogle : function(data, lat, lon) {
    for (let e of data) {
      if (e[lat] && e[lon]) {
        let arr = this.gcj_encrypt(Number(e[lat]), Number(e[lon]));
        e[lat] = arr['lat'];
        e[lon] = arr['lon'];
      }
    }
  },
  gpsToGoogleForList : function(data) {
    for (let e of data) {
      for (let a of e) {
        if (!isNaN(a[0]) && !isNaN(a[1])) {
          let arr = this.gcj_encrypt(Number(a[0]), Number(a[1]));
          a[0] = arr['lat'];
          a[1] = arr['lon'];
        }
      }
